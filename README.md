...
Nothing in the repo-hosting work depends on it, and it isn't fixed here
because the fix is a schema decision (make the column nullable and
migrate, or pin better-auth) that changes the auth tables. **Sign-up is
broken until one of those is chosen.**
